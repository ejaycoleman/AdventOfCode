...
for (var i = 0; i < values.length; i++) {
  var row = 0;
  for (var j = 0; j < values[i].length; j++) {
    for (var k = j; k < values[i].length; k++) {
      if (values[i][j] % values[i][k] == 0) {

        row = values[i][j] / values[i][k];

      } else if (values[i][k] % values[i][j] == 0) {
        row = values[i][k] / values[i][j];
      }
    }
    //console.log(row);
    if (row != 1) {
